...
void global_init() {
    ...
    for (uint32_t va = FRAME_SIZE; va < kConfig.memSize; va += FRAME_SIZE) {
        ...
        auto pde = shared[pdi];
        if ((pde & 1) == 0) {
            pde = PhysMem::alloc_frame() | 0x3;
            shared[pdi] = pde;
        }
        auto pt = (uint32_t*)(pde & 0xFFFFF000);
        pt[pti] = va | 0x3;
        //map(shared,va,va, false);
    }
...
extern "C" void vmm_pageFault(uintptr_t va_, uintptr_t *saveState) {
    ...
    if (va_ < 0x80000000 || is_special(va)) {
        me->process->exit(-1);
        stop();
        return;
    }else {
        auto pdi = va >> 22;
        auto pti = (va >> 12) & 0x3FF;
        auto pde = me->process->pd[pdi];
        if ((pde & 1) == 0) {
            pde = PhysMem::alloc_frame() | 0x7;
            me->process->pd[pdi] = pde;
        }
        auto pt = (uint32_t*)(pde & 0xFFFFF000);
